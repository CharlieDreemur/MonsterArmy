...
#region Property访问器
    ...
    public DamageInfo GetAttackDamageInfo(){
        int atkDamage = attributeData.fixedData.atk.FinalAttribute;
        if(IsCrit()){
            atkDamage = (int)(atkDamage*attributeData.fixedData.CritDamage);
        }
        DamageInfo toReturn = new DamageInfo(atkDamage, entity);
        return toReturn;
    }
    ...
    */
    public static int CalculateDamage(EntityAttribute attacker_Attr, EntityAttribute target_Attr, out bool IsDodge, out Enum_DamageType damageType){
        IsDodge = false;
